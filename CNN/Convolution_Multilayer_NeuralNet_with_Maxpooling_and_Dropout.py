'''
This file implements a multi layer neural network for a multiclass classifier with One Convolution layer and one Max pooling layer. This also implements Dropout.
...
Accuracy for training set is 37.617 %
Accuracy for testing set is 29.400 %
'''
import numpy as np
...
def Dropout_forward(Alprev, p):
    P = np.random.randn(Alprev.shape[0], Alprev.shape[1])
    P = P < p
    Al = np.multiply(Alprev, P)
    Al = Al / p
    return Al, P

def Dropout_backward(dAl, P, p):
    dAlprev = np.multiply(dAl, P)
    dAlprev = dAlprev / p
    return dAlprev
...
def multi_layer_forward(X, parameters):
    ...
    caches = []
    for l in range(1,L): 
        A, P = Dropout_forward(A, p) # since there is no W0 and b0
        A, cache = layer_forward(A, parameters["W"+str(l)], parameters["b"+str(l)], "relu")        
        Dropout_cache["P"+str(l)] = P
        caches.append(cache)
...
def multi_layer_backward(dAL, caches, parameters):
    ...
    for l in reversed(range(1,L+1)):
        dA, gradients["dW"+str(l)], gradients["db"+str(l)] = \
                    ...
                    activation)
        activation = "relu"
        if "P"+str(l) in Dropout_cache.keys():
            dA = Dropout_backward(dA, Dropout_cache["P"+str(l)], p)
...
def main():
    ...
    plt.title('Convolution Multi Layer Multi class Classifier with Max pooling and Drop out')
    plt.savefig('Convolution Multi_Layer_Multi_Class_Classifier_Max_pooling_Dropout.jpg')
