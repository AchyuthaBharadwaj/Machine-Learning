...
def softmax_cross_entropy_loss(Z, Y=np.array([])):
    ...
    C = np.sum(np.exp(Z - Zmax), axis=0, keepdims = True)
    A = np.exp(Z - Zmax) / C  
    logA = np.log(A)
    Y_bar = onehot_encoder.fit_transform(Y.T)    
    loss =  -np.sum(Y_bar.T * logA)
    cache = {}
    cache["A"] = A
    return A, cache, loss
...
def main():
    ...
    plt.title('Multi Layer Multi class Classifier')
    plt.savefig('Multi_Layer_Multi_Class_Classifier.png')
